...
export default function EditUser({ customerData, setCustomerData, userData }) {
   ...
   const { access_level, account_id, billing_rate, cost_rate, created_at, display_name, email, is_user_active, job_title, user_id } = userData || {};
   ...
   return (
      <>
         <Stack spacing={3} sx={{ marginTop: '25px' }}>
            ...
            <FormControlLabel
               control={<Checkbox checked={selectedItems.isUserActive} onChange={e => setSelectedItems(otherItems => ({ ...otherItems, isUserActive: e.target.checked }))} />}
               label='Active User'
            />
