...
accountRouter.route('/updateAccount').put(jsonParser, async (req, res) => {
  const db = req.app.get('db');
  // Sanitize fields
...
accountRouter.route('/getAccount/:accountID').get(async (req, res) => {
  const db = req.app.get('db');
...
// Delete account
accountRouter.route('/deleteAccount/:accountID').delete(async (req, res) => {
  const db = req.app.get('db');
  const { accountID } = req.params;
