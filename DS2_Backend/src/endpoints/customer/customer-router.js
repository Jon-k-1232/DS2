...
customerRouter.route('/updateCustomerInformation').put(jsonParser, async (req, res) => {
  const db = req.app.get('db');
...
// Delete Customer
customerRouter.route('/deleteCustomer/:customerID/:accountID').delete(jsonParser, async (req, res) => {
  const db = req.app.get('db');
