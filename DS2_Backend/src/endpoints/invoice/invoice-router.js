...
invoiceRouter.route('/createInvoice/:accountID/:userID').post(jsonParser, async (req, res) => {
     ...
     const invoiceQueryData = await fetchInitialQueryItems(db, invoicesToCreate, accountID);
     const calculatedInvoices = calculateInvoices(invoicesToCreate, invoiceQueryData);
     // const formInvoiceObjects = formInvoiceObjects(invoicesToCreate, invoiceCreationSettings);
