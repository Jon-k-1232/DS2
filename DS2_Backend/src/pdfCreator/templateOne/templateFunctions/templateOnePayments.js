...
const createPaymentsSection = (doc, invoiceDetails, preferenceSettings) => {
  ...
  Object.values(payments?.paymentRecords ?? {}).forEach((paymentRecord, index) => {
    ...
    if (index === Object.values(payments?.paymentRecords ?? {}).length - 1) {
      doc
        .lineCap('butt')
        .lineWidth(1)
      ...
      doc
        ...
        .fontSize(12)
        .text(
          `Total Payments: ${payments.paymentTotal.toFixed(2)}`,
          alignRight(`Total Payments: ${payments.paymentTotal.toFixed(2)}`, 1),
  ...
  if (!Object.values(payments?.paymentRecords ?? {}).length) {
    doc
      .lineCap('butt')
      .lineWidth(1)
