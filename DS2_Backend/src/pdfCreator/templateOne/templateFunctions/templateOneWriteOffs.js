const createWriteOffsSection = (doc, invoiceDetails, preferenceSettings) => {
  ...
  writeOffs.writeOffs.forEach((record, index) => {
    ...
    if (index === writeOffs.writeOffs.length - 1) {
      ...
      doc
        ...
        .fontSize(12)
        .text(
          `Total Revisions: ${writeOffs.totalWriteOffs.toFixed(2)}`,
          alignRight(`Total Revisions: ${writeOffs.totalWriteOffs.toFixed(2)}`, 1),
