const createChargesSection = (doc, invoiceDetails, preferenceSettings) => {
  ...
  Object.values(transactions?.jobs ?? {}).forEach((record, index) => {
    ...
    if (index === Object.values(transactions?.jobs ?? {}).length - 1) {
      ...
      doc
        ...
        .fontSize(12)
        .text(
          `Total New Charges: ${transactions.transactionTotals.toFixed(2)}`,
          alignRight(`Total New Charges: ${transactions.transactionTotals.toFixed(2)}`, 1),
  ...
  if (!Object.values(transactions?.jobs ?? {}).length) {
    doc
      .lineCap('butt')
      .lineWidth(1)
